...
int main() {
  ...
  char * name3 = "Player3";
  char * name4 = "Player4";
  player_t * pfirst = createPlayer(name1, first);     //player is a struct
  player_t * psecond = createPlayer(name2, first);  //doubly linked list as
  player_t * pthird = createPlayer(name3, first);     // initially designed
  player_t * pfourth = createPlayer(name4, first);
  node_t * node1 = createNode(pfirst);
  node_t * node2 = createNode(psecond);
  node_t * node3 = createNode(pthird);
  node_t * node4 = createNode(pfourth);
  ...
  printf("Congrats, %s wins!\n", currentplayer->name); //which of four players wins
...
node_t * findColour(char*colour, node_t*position){
  if (position->next == NULL){
    return position;        // let color on the board is the position
  }
  else {
